...
const SFX_BUS := &"SFX"
const DUNGEON_REVERB_RESOURCE_NAME := "Dungeon SFX Reverb"
const DUNGEON_REVERB_ROOM_SIZE := 0.8
const DUNGEON_REVERB_DAMPING := 0.75
const DUNGEON_REVERB_PREDELAY_MSEC := 45.0
const DUNGEON_REVERB_PREDELAY_FEEDBACK := 0.12
const DUNGEON_REVERB_SPREAD := 0.9
const DUNGEON_REVERB_HIPASS := 0.2
const DUNGEON_REVERB_DRY := 1.0
const DUNGEON_REVERB_WET := 0.16


## Gives gameplay sound effects a restrained stone-dungeon acoustic without changing music.
static func setup_dungeon_sfx_environment() -> void:
	var sfx_bus_index := AudioServer.get_bus_index(SFX_BUS)
	if sfx_bus_index < 0:
		push_warning("Cannot configure dungeon acoustics because the SFX audio bus is missing.")
		return

	var effect_index := _find_dungeon_reverb_effect_index(sfx_bus_index)
	var reverb: AudioEffectReverb
	if effect_index >= 0:
		reverb = AudioServer.get_bus_effect(sfx_bus_index, effect_index) as AudioEffectReverb
	if reverb == null:
		if effect_index >= 0:
			AudioServer.remove_bus_effect(sfx_bus_index, effect_index)
		reverb = AudioEffectReverb.new()
		reverb.resource_name = DUNGEON_REVERB_RESOURCE_NAME
		AudioServer.add_bus_effect(sfx_bus_index, reverb)

	reverb.room_size = DUNGEON_REVERB_ROOM_SIZE
	reverb.damping = DUNGEON_REVERB_DAMPING
	reverb.predelay_msec = DUNGEON_REVERB_PREDELAY_MSEC
	reverb.predelay_feedback = DUNGEON_REVERB_PREDELAY_FEEDBACK
	reverb.spread = DUNGEON_REVERB_SPREAD
	reverb.hipass = DUNGEON_REVERB_HIPASS
	reverb.dry = DUNGEON_REVERB_DRY
	reverb.wet = DUNGEON_REVERB_WET


## Restores dry sound effects when leaving the dungeon gameplay scene.
static func clear_dungeon_sfx_environment() -> void:
	var sfx_bus_index := AudioServer.get_bus_index(SFX_BUS)
	if sfx_bus_index < 0:
		return
...
static func _find_dungeon_reverb_effect_index(sfx_bus_index: int) -> int:
	for effect_index in AudioServer.get_bus_effect_count(sfx_bus_index):
		var effect := AudioServer.get_bus_effect(sfx_bus_index, effect_index)
		if effect.resource_name == DUNGEON_REVERB_RESOURCE_NAME:
			return effect_index
