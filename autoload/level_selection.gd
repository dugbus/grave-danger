...
func get_selected_level_scene() -> PackedScene:
	var scene_path := get_selected_level_scene_path()
	if scene_path.is_empty():
		return null

	var scene_loader := get_node_or_null("/root/SceneLoader") as SCENE_LOADER_SCRIPT
	if scene_loader != null:
		var cached_scene := scene_loader.get_cached_scene(scene_path)
		if cached_scene != null:
			return cached_scene
	return load(scene_path) as PackedScene


## Returns the selected scene path without performing resource loading.
func get_selected_level_scene_path() -> String:
	return get_level_scene_path(selected_level_index)


## Returns a level scene path without performing resource loading.
func get_level_scene_path(index: int) -> String:
	if level_mapping == null:
		return ""
	return level_mapping.get_level_scene_path(index)
