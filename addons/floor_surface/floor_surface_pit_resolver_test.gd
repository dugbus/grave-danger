...
func _test_open_void_is_not_a_pit() -> void:
	var floor_map := MAP_SCRIPT.new()
	floor_map.dimensions = Vector2i(5, 3)
	floor_map.default_present = true
	floor_map.presence_exceptions = [
		Vector2i(0, 1),
		Vector2i(1, 1),
		Vector2i(3, 1),
	]
	...
	var result := SUBJECT.new().resolve(floor_map, profile, styles)
	var bottoms := result["bottom_heights"] as Dictionary[Vector2i, float]
	expect_equal(result["region_count"], 1, "Only the enclosed empty region is a pit.")
	expect(
		not bottoms.has(Vector2i(0, 1)) and not bottoms.has(Vector2i(1, 1)),
		"Empty space connected to an authored boundary remains an exterior void."
	)
	expect(bottoms.has(Vector2i(3, 1)), "An enclosed hole still receives its shared wall datum.")
