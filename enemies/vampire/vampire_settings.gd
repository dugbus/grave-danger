...
## Maximum hunting speed used by the Vampire's route following.
@export_range(0.1, 20.0, 0.05) var max_speed := 5.0
...
@export_range(1.0, 100.0, 0.5) var sight_distance := 12.0
## Total horizontal field of view used to visually acquire the player.
@export_range(0.0, 360.0, 1.0, "suffix:°") var sight_field_of_view_degrees := 110.0
...
@export_range(0.0, 1.0, 0.05) var proximity_fog_max_intensity := 0.9
...
@export_subgroup("Obstacle Stride")
...
@export_subgroup("Route Recovery")
...
@export_subgroup("Last Seen Prediction")
...
@export_subgroup("Corridor Looks")
## Seconds the moving Vampire keeps its head aimed into a newly encountered side corridor.
@export_range(0.1, 3.0, 0.05) var junction_scan_seconds_per_direction := 0.75
## Floor distance checked while approaching a possible side corridor during a search route.
@export_range(0.5, 8.0, 0.25) var junction_scan_probe_distance := 2.0

@export_subgroup("Noise Search")
...
@export_subgroup("Layout Knowledge")
