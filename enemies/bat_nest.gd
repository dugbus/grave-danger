...
func _update_swarming(delta: float) -> void:
	...
	for bat_index in bats.size():
		...
		var bat_node_position := bat_state.node.global_position
		var target_position := player_center + _get_harass_offset(bat_state, bat_index)
		var desired_to_target := target_position - bat_node_position
		...
		var cohesion := center - bat_node_position
		...
		acceleration += _get_separation(bat_node_position, bat_state) * separation_weight
...
func _get_separation(bat_position: Vector3, current_bat: BatState) -> Vector3:
	var separation := Vector3.ZERO
	for other_bat in bats:
		if other_bat == current_bat or other_bat.node == null:
			continue

		var offset := bat_position - other_bat.node.global_position
